...
def download_image(image_url, dst_dir, file_name, timeout=10, proxy_type=None, proxy=None):
    proxies = None
...
def download_images(image_urls, dst_dir, file_prefix="img", concurrency=128, timeout=1, proxy_type=None, proxy=None):
    """
    Download image according to given urls and automatically rename them in order.
    :param timeout:
    :param proxy:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as executor:
        ...
        for image_url in image_urls:
            ...
            count += 1
        concurrent.futures.wait(future_list, timeout=1)
