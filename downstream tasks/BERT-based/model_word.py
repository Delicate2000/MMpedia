...
class WordDataset(Dataset):
  def __init__(self, data_file, data_dir, with_image=False, image_token_num=2, max_length=60):
  ...
  def __getitem__(self, idx):
    ...
    if self.with_image:
      bert_token = self.tokenizer(text, 
      ...
      input_ids = bert_token['input_ids'][0]
      attention_mask = bert_token['attention_mask'][0]

      img = Image.open(image_path).convert('RGB')
      img_t = transform(img)
      
      return text, o, input_ids, attention_mask, img_t
    else:
      bert_token = self.tokenizer(text, 
      ...
      input_ids = bert_token['input_ids'][0] # 32 * 28
      attention_mask = bert_token['attention_mask'][0]


      return text, o, input_ids, attention_mask

# pytorch dataloader
      
class WordModel(nn.Module):
  def __init__(self, encoder_hidden_dim, device, maxlength=60, image_token_num=2, 
    ...
    self.bert_model = BertModel.from_pretrained("../bert-base-uncased")   # [mask]   30522gehouxuanci  -> unk
    ...
    self.with_image = with_image
    if with_image:
      self.imageProcessor = ImageFeature(self.device, 'ResNet50')
      self.image_token_num = image_token_num
  ...
  def forward(self, text, o, input_ids, attention_mask, image=None):
    ...
    if self.with_image:
      ...
      image_token = image_hidden.view(bs, self.image_token_num, -1) # 32 * 2 * 768
      
      bert_emb = self.tokenizer_embeddings(input_ids) # 32 * 28 * 768 
      bert_emb = torch.cat([image_token, bert_emb], dim=1) # 32 * (28 + 2) * 768
      ...
      bert_emb = self.tokenizer_embeddings(input_ids) # 32 * maxlength(30) * 768
      mask_pos = torch.nonzero(input_ids == self.tokenizer.mask_token_id, as_tuple=True)[1]

    output = self.bert_model(inputs_embeds=bert_emb, attention_mask=attention_mask)[0]
...
def train():
  ...
  valid_loader = DataLoader(valid_data, batch_size=args.batch_size, shuffle=False, num_workers=24)
  ...
  for epoch in range(args.epochs):
    ...
    if best_cnt < cnt:
      best_cnt = cnt
      print('start test!')
      test_mrr, test_mr, test_hits1, test_hits3, test_hits10 = test(baseModel)
