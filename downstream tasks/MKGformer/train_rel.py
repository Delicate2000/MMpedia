...
#             print("bsz:",bsz)
#             print("torch.arange(bsz):", torch.arange(bsz))
            # if mode != 'predict_rel':
        ...
        # if hits1> valid_hits1: # 大的模型才有可能保存被预测
        if hits10> valid_hits10: # 大的模型才有可能保存被预测
            # valid_hits1 = hits1
            valid_hits10 = hits10
    ...
    print('------------final_result------------------') # 打印的test不是best无所谓
    ...
    with open('loop_result.txt',"a") as f1:
        f1.write('noise:{}, seed_value:{}, batch_size:{} \n mrank:{}, mrr:{}, hits1:{}, hits3:{}, hits10:{} \n'\
        .format(str(noise), str(seed_value), str(batch_size), str(test_MR), str(test_MRR), str(test_hits1), str(test_hits3), str(test_hits10)))
...
def _eval(model, eval_data, eval_type):
    ...
    with torch.no_grad():
        ...
        for query, rel, ans, input_ids, token_type_ids, attention_mask, input_img, aux_image, label in tqdm(eval_data):
            ...
            _, outputs = torch.sort(logits, dim=1, descending=True) # bsz, entities   index 先得到所有的排序
            _, outputs = torch.sort(outputs, dim=1) # 索引label所在排序
            # print("logits:", logits)
            # print("label:", label)
