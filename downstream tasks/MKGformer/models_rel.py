...
class CLIPVisionEmbeddings(nn.Module):
    ...
    def forward(self, pixel_values, aux_embeddings=None, rcnn_embeddings=None):
        ...
        aux_embeds = self.patch_embedding(aux_embeddings)  # shape = [*, width, grid, grid]
        aux_embeds = aux_embeds.flatten(2).transpose(1, 2)  # shape = [*, grid*grid, width]
...
class UnimoForMaskedLM(nn.Module):
    def __init__(self, vision_config, text_config, pretrain=False, vocab_size=7796):
        super().__init__()
        self.unimo = UnimoModel(vision_config, text_config)
        self.vocab_size = vocab_size
        self.cls = UnimoOnlyMLMHead(text_config, self.vocab_size)
    ...
    def forward(
        ...
        attention_mask=None,
        token_type_ids=None,
        position_ids=None, # (1, len position emb) is contiguous in memory and exported when serialized
        head_mask=None, # head_mask (:obj:`torch.Tensor` with shape :obj:`[num_heads]` or
        
        pixel_values=None,
        aux_values=None, 
        rcnn_values=None,
        output_attentions=None, # bool
        output_hidden_states=None, # bool CLIP返回隐藏？
        return_dict=None,
        labels=None,
    ):
        outputs = self.unimo(
...
class UnimoOnlyMLMHead(nn.Module):
    def __init__(self, config, vocab_size):
        super().__init__()
        self.predictions = UnimoLMPredictionHead(config, vocab_size)
...
#         print("init:", self.decoder)
#         self.bias = nn.Parameter(torch.zeros(vocab_size))
#         self.decoder.bias = self.bias

    def forward(self, hidden_states):
#         print("input_hidden_states.size:",hidden_states.size())
        hidden_states = self.transform(hidden_states)
#         print("hidden_states.size:",hidden_states.size())
        
#         print("use:",self.decoder)
...
#         print("outputs:",outputs[0])
        sequence_output = outputs[0][:,0,:] # 拿cls
        prediction_scores = self.cls(sequence_output)
