...
class ComplExMDR_REC(KBCModel):
    ...
    def __init__(
        ...
        super(ComplExMDR_REC, self).__init__()
    ...
    def score(self, x):
        lhs = self.r_embeddings[0](x[:, 2])
        rel = self.r_embeddings[1](x[:, 1])
        rhs = self.r_embeddings[0](x[:, 0])
        ...
        score_str = torch.sum(
            (lhs[0] * rhs[0] - lhs[1] * rhs[1]) * rel[0] +
            (lhs[0] * rhs[1] + lhs[1] * rhs[0]) * rel[1],
            1, keepdim=True
        ...
        if self.modality_split:
            if self.fusion_img:
                ...
                lhs_i = str_img_embeddings[(x[:, 2])]  # 找对应的embedding 前面算了全部的
                rel_i = self.img_rel_embeddings(x[:, 1])
                rhs_i = str_img_embeddings[(x[:, 0])]

                lhs_i = lhs_i[:, :self.rank], lhs_i[:, self.rank:]
                rel_i = rel_i[:, :self.rank], rel_i[:, self.rank:]
                rhs_i = rhs_i[:, :self.rank], rhs_i[:, self.rank:]
                score_img = torch.sum(
                    (lhs_i[0] * rhs_i[0] - lhs_i[1] * rhs_i[1]) * rel[0] +
                    (lhs_i[0] * rhs_i[1] + lhs_i[1] * rhs_i[0]) * rel[1],
                    1, keepdim=True
                )  # ComplEx的score function
                # [batch , 1]
            ...
            if self.fusion_dscp:
                ...
                lhs_i = str_dscp_embeddings[(x[:, 2])] # 找对应的embedding 前面算了全部的
                rel_i = self.dscp_rel_embeddings(x[:, 1])
                rhs_i = str_dscp_embeddings[(x[:, 0])]
    ...
    def forward(self, x):
        ...
        lhs = self.r_embeddings[0](x[:, 2]) # 头 处理时候反了
        rel = self.r_embeddings[1](x[:, 1])
        rhs = self.r_embeddings[0](x[:, 0]) # 尾巴
        ...
        to_score = self.r_embeddings[1].weight
        ...
        score_str = (
                (lhs[0] * rhs[0] - lhs[1] * rhs[1]) @ to_score[0].transpose(0, 1) +
                (lhs[0] * rhs[1] + lhs[1] * rhs[0]) @ to_score[1].transpose(0, 1)
        )
        ...
        if self.modality_split:
            if self.fusion_img:
                ...
                score_img = (
                        (lhs_i[0] * rhs_i[0] - lhs_i[1] * rhs_i[1]) @ to_score_i[0].transpose(0, 1) +
                        (lhs_i[0] * rhs_i[1] + lhs_i[1] * rhs_i[0]) @ to_score_i[1].transpose(0, 1)
                )
            ...
            if self.fusion_dscp:
                ...
                str_dscp_embeddings = (1 - self.alpha) * self.r_embeddings[0].weight + self.alpha * dscp_embeddings
                str_dscp_embeddings /= self.temp
                lhs_d = str_dscp_embeddings[(x[:, 2])]
                rel_d = self.dscp_rel_embeddings(x[:, 1])
                rhs_d = str_dscp_embeddings[(x[:, 0])]
                ...
                score_dscp = (
                        (lhs_d[0] * rhs_d[0] - lhs_d[1] * rhs_d[1]) @ to_score_d[0].transpose(0, 1) +
                        (lhs_d[0] * rhs_d[1] + lhs_d[1] * rhs_d[0]) @ to_score_d[1].transpose(0, 1)
                )
                factors_dscp = (
    ...
    def get_rhs(self, chunk_begin: int, chunk_size: int):
        rhs_str = self.r_embeddings[1].weight[
                  chunk_begin:chunk_begin + chunk_size
                  ].transpose(0, 1) #维度转置
        return_value = []
    ...
    def get_queries(self, queries: torch.Tensor):
        ...
        lhs = embedding[(queries[:, 2])]
        rhs = embedding[(queries[:, 0])]
        ...
        if self.modality_split:
            if self.fusion_img:
                ...
                lhs_i = str_img_embeddings[(queries[:, 2])]
                rhs_i = str_img_embeddings[(queries[:, 0])]

                lhs_i = lhs_i[:, :self.rank], lhs_i[:, self.rank:]
                rhs_i = rhs_i[:, :self.rank], rhs_i[:, self.rank:]

                queries_img = torch.cat([
                    lhs_i[0] * rhs_i[0] - lhs_i[1] * rhs_i[1],
                    lhs_i[0] * rhs_i[1] + lhs_i[1] * rhs_i[0]
                ], 1)
            ...
            if self.fusion_dscp:
                ...
                str_dscp_embeddings = (1 - self.alpha) * self.r_embeddings[0].weight + self.alpha * dscp_embeddings
                str_dscp_embeddings /= self.temp
                lhs_d = str_dscp_embeddings[(queries[:, 2])]
                rhs_d = str_dscp_embeddings[(queries[:, 0])]

                lhs_d = lhs_d[:, :self.rank], lhs_d[:, self.rank:]
                rhs_d = rhs_d[:, :self.rank], rhs_d[:, self.rank:]

                queries_dscp = torch.cat([
                    lhs_d[0] * rhs_d[0] - lhs_d[1] * rhs_d[1],
                    lhs_d[0] * rhs_d[1] + lhs_d[1] * rhs_d[0]
                ], 1)
    ...
    def get_ranking(
        ...
        if chunk_size < 0:
            chunk_size = self.sizes[1]  # eval时每次candidate都是所有实体 （chuck_size default = -1）
                ...
                while b_begin < len(queries):  # batch iter
                    if not self.modality_split or (
                        ...
                        for i, query in enumerate(these_queries):
                            # filter_out = filters[(query[0].item(), query[1].item())]
                            # filter_out += [queries[b_begin + i, 2].item()]
                            filter_out = [queries[b_begin + i, 1].item()]
                        ...
                        for i, query in enumerate(these_queries):
                            # filter_out = filters[(query[0].item(), query[1].item())] # 暂时无效可以做rel的filter
                            
                            # classification_report 评价用这个 
        ...
        if not self.modality_split or (self.modality_split and not self.fusion_img and not self.fusion_dscp):
            ...
            if self.fusion_img and self.fusion_dscp:
                ...
                ranks_fusion = torch.min(ranks_str, ranks_img) # 三个最小的 正确？
            elif self.fusion_dscp:
