...
def meta_test(model, inputs, batch_size=256):
    ...
    output = torch.zeros((inputs.shape[0], inputs.shape[1])).squeeze().to(device)  # T,E
    b_begin = 0
    while b_begin < inputs.shape[0]:
        # inputs_batch = inputs[b_begin: b_begin + batch_size].cuda()
        inputs_batch = inputs[b_begin: b_begin + batch_size].to(device)
...
def main():
    ...
    train_valid_examples = torch.from_numpy(train_valid.astype('int64')).to(device)
    test = dataset.get_examples('test')
    # test_examples = torch.from_numpy(test.astype('int64')).cuda()
    test_examples = torch.from_numpy(test.astype('int64')).to(device)
    valid = dataset.get_examples('valid')
    # valid_examples = torch.from_numpy(valid.astype('int64')).cuda()
    valid_examples = torch.from_numpy(valid.astype('int64')).to(device)
    ...
    for e in range(max_epochs):
        ...
        if metrics['hits@[1,3,10]'][2] > best_hits10:
            ...
            normal_test_mr = meta_test_mr.copy()
            normal_test_mrr = meta_test_mrr.copy()
            normal_test_hits = meta_test_hits.copy()
            normal_epoch = e

    print("normal_result:")
    print(normal_test_mr)
    print(normal_test_mrr)
    print(normal_test_hits)
    print("normal_epoch:", normal_epoch)

big_datasets = ['WN9', 'WN18', 'FB15K-237', 'DB15K']
