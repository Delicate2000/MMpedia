...
class ComplExMDR(KBCModel):
    ...
    def __init__(
        ...
        if self.fusion_img:
            ...
            if device != 'cpu':
                ...
                self.img_vec = torch.from_numpy(self.img_info).float().to(device)
            else:
        ...
        if self.fusion_dscp:
            ...
            if device != 'cpu':
                ...
                self.dscp_vec = torch.from_numpy(self.dscp_info).float().to(device)
            else:
                self.dscp_vec = torch.from_numpy(self.dscp_info).float()
    ...
    def score(self, x):
        ...
        if self.modality_split:
            if self.fusion_img:
                ...
                lhs_i = str_img_embeddings[(x[:, 0])]  # 找对应的embedding 前面算了全部的
            ...
            if self.fusion_dscp:
                ...
                lhs_i = str_dscp_embeddings[(x[:, 0])] # 找对应的embedding 前面算了全部的
    ...
    def forward(self, x):
        # score为lhs rel时预测 尾巴。换这个来预测头和关系 truth也变

        lhs = self.r_embeddings[0](x[:, 0])
        rel = self.r_embeddings[1](x[:, 1])
        rhs = self.r_embeddings[0](x[:, 2])

        # print("r_embeddings:", self.r_embeddings)

        lhs = lhs[:, :self.rank], lhs[:, self.rank:]  # 根据rank区分
        rel = rel[:, :self.rank], rel[:, self.rank:]
        rhs = rhs[:, :self.rank], rhs[:, self.rank:]
        ...
        # print("return_value[0]:", return_value[0][0]) # 没softmax

        # for i in range(len(return_value[1])):
        #     print("return_value[1]:", return_value[1][i].size())
        return tuple(return_value)
    ...
    def get_rhs(self, chunk_begin: int, chunk_size: int):
        ...
        # print("chunk_begin+chunk_size:", chunk_begin+ chunk_size)
        # print("\n\n-----------------")

        # print("self.r_embeddings[0].weight.size():", self.r_embeddings[0].weight.size())
    ...
    def get_queries(self, queries: torch.Tensor):
        # 这里lhs和rel运算 再带上get_rhs返回的embedding权重 等于forward
        # 改的话把 get_queries, get_ranking, get_rhs, score得换成对应的rel
    ...
    def get_ensemble_score_filtered_ranking(
        ...
        with torch.no_grad():
            ...
            while c_begin < self.sizes[2]:
                ...
                pbar = tqdm(total=len(queries), ncols=80)
                while b_begin < len(queries):  # batch iter
                    these_queries = queries[b_begin:b_begin + batch_size].to(device)
                    # these_scores = ensemble_score[b_begin:b_begin + batch_size].clone().cuda()
                    these_scores = ensemble_score[b_begin:b_begin + batch_size].clone().to(device)
                    # these_modalities = mod_tri_weight[:, b_begin:b_begin + batch_size, :].clone().cuda()  # 3, batch, 1
                    these_modalities = mod_tri_weight[:, b_begin:b_begin + batch_size, :].clone().to(device)
                    targets_str, targets_img, targets_dscp = self.score(these_queries)  # batch,1
    ...
    def get_ranking(
        ...
        with torch.no_grad():
            ...
            while c_begin < self.sizes[2]: # 这循环只执行一次
                b_begin = 0
                pbar = tqdm(total=len(queries), ncols=80)
                while b_begin < len(queries):  # batch iter
                    if not self.modality_split or (
                        ...
                        these_queries = queries[b_begin:b_begin + batch_size]
                        rhs = self.get_rhs(c_begin, chunk_size)[0] # c_begin是开始数
                        q = self.get_queries(these_queries)[0]
                        scores_str = q @ rhs
                        scores = scores_str  # torch.Size([500, 14951])
                        # print("scores:",scores.size())
                        see = self.score(these_queries)
                        targets = see[0]
                        # targets = self.score(these_queries)[0]
                        # print('-------------------\n\n')
                        # print("self.score(these_queries):", see)
                        # print("targets:", targets)
                        # print("targets.size():", targets.size())
                        # print('-------------------\n\n')
                        for i, query in enumerate(these_queries):
                            filter_out = filters[(query[0].item(), query[1].item())] # 出现在train, valid, test中出现都算对，为啥label会不在这里
                            if queries[b_begin + i, 2].item() not in filter_out:
                                print("wrong")
                            filter_out += [queries[b_begin + i, 2].item()]
        ...
        with torch.no_grad():
            ...
            while c_begin < self.sizes[2]:
                ...
                while b_begin < len(queries):  # batch iter
                    ...
                    these_scores = ensemble_score[b_begin:b_begin + batch_size].clone()
                    targets_ensemble = ensemble_target[b_begin:b_begin + batch_size].clone() # batch
...
# alpha=0.7
# beta=0.5
# constant=True  #Use constants to simplify alpha
# random_gate=False  #Use random gate
# forget_gate=True   #user forget gate
# remember_rate=100
# alpha=1的时候不用图片
...
# round取整 sig_alpha img权重   rel_pd 0 1 img要不要
# class RSME(KBCModel):
#     def __init__(
#             self, sizes: Tuple[int, int, int], rank: int,
#             alpha,
#             beta,
#             init_size: float = 1e-3,
#             img_info='../data/DB15K/img_vec.pickle',
...
#         # embedding应该是索引 没有计算 原来也对
#         # lhs = embedding[(x[:, 0])]
#         # rel = self.r_embeddings[1](x[:, 1])
#         # rhs = embedding[(x[:, 2])]

#         lhs = (1 - self.alpha) * self.r_embeddings[0](x[:, 0]) + self.alpha* img_embeddings[(x[:, 0])]
#         rel = self.r_embeddings[1](x[:, 1])
#         rhs = (1 - self.alpha) * self.r_embeddings[0](x[:, 2]) + self.alpha * img_embeddings[
#                 (x[:, 2])]
...
#             # ans = torch.ones(0, self.r_embeddings[0].weight.size(0))
...
#         img_embeddings = self.img_vec.mm(self.post_mats)
#         # if not constant:
#         lhs = (1 - self.alpha) * self.r_embeddings[0](queries[:, 0]) + self.alpha* img_embeddings[(queries[:, 0])]
#         rel = self.r_embeddings[1](queries[:, 1])
...
class RSME(KBCModel):
    def __init__(
        ...
        self.r_embeddings[1].weight.data *= init_size
        if not constant:
            self.alpha = torch.from_numpy(pickle.load(open(sig_alpha, 'rb')))
            self.alpha = torch.cat((self.alpha, self.alpha), dim=0)
        else:
            self.alpha = nn.Parameter(torch.tensor(alpha), requires_grad=False)  # [14951, 2000]
        self.img_dimension = 1000
        self.img_info = pickle.load(open(img_info, 'rb'))
        self.img_info *= init_size # 加的
        if device != 'cpu':
            # self.img_vec = torch.from_numpy(self.img_info).float().cuda()
            self.img_vec = torch.from_numpy(self.img_info).float().to(device)
        else:
        ...
        # if not random_gate:
        #     self.rel_pd = torch.from_numpy(pickle.load(open(rel_pd.format(remember_rate), 'rb')))
        # else:
        #     tmp = pickle.load(open(rel_pd.format(remember_rate), 'rb'))
        #     self.rel_pd = torch.from_numpy(sc_wz_01(len(tmp), np.sum(tmp))).unsqueeze(1)
        ...
        self.post_mats = nn.Parameter(torch.Tensor(self.img_dimension, 2 * rank), requires_grad=True)
        # self.post_mats = nn.Linear(self.img_dimension, 2 * rank)
        nn.init.xavier_uniform(self.post_mats)

        # layer_norm1 = nn.LayerNorm(2 * rank)
        # layer_norm2 = nn.LayerNorm(2 * rank)
    ...
    def score(self, x):
        ...
        img_embeddings = self.img_vec.mm(self.post_mats)
        # img_embeddings = self.post_mats(self.img_vec)
        if not constant:
            lhs = (1 - self.alpha[(x[:, 1])]) * self.r_embeddings[0](x[:, 0]) + self.alpha[(x[:, 1])] * img_embeddings[
                (x[:, 0])]
            rel = self.r_embeddings[1](x[:, 1])
            ...
            while c_begin < self.sizes[2]:
                ...
                while b_begin < len(queries):
                    ...
                    if not constant:
                        ...
                        h_r = self.get_queries(these_queries)
                        n = len(h_r)
                        scores_str = torch.ones(0, self.r_embeddings[0].weight.size(0))
                    ...
                    # print("scores:", scores)
                    # print("scores:", scores[0, queries[b_begin, 2].item()])
