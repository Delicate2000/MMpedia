...
from datasets_relation import Dataset
from models_relation import CP, ComplEx, RSME, ComplExMDR, RotateE
from models_REC import ComplExMDR_REC
from regularizers import F2, N3
from optimizers_rel import KBCOptimizer
...
models = ['CP', 'ComplEx', 'RSME', 'ComplExMDR', 'RotateE']
parser.add_argument(
...
for e in tqdm(range(args.max_epochs)):
    ...
    if (e + 1) % args.valid == 0:
        split = 'train'
        train = {}
        mean_rank, mean_reciprocal_rank, hits_at = dataset.eval(model, split, -1 if split != 'train' else 3000)  # eval里get_rank avg_both求了平均
        train['MRR'] = mean_rank
        ...
        # if valid['hits@[1,3,10]'][0] > best_hits[0]:
            best_mrr = valid['MRR']
            best_mr = valid['MR']
