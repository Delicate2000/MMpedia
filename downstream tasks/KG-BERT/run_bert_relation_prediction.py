...
class KGProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type, data_dir):
        ...
        for (i, line) in tqdm(enumerate(lines)):
            guid = "%s-%s" % (set_type, i)
            text_a = line[0].replace('_', ' ') + ' ' + entity2text[line[0]]
            text_b = line[2].replace('_', ' ') + ' ' + entity2text[line[2]]
            label = line[1].replace('_', ' ')
...
if __name__ == "__main__":
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        eval_examples = processor.get_dev_examples(args.data_dir) # valid
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        eval_examples = processor.get_test_examples(args.data_dir) # 获取测试集
        eval_features = convert_examples_to_features(
            eval_examples, label_list, args.max_seq_length, tokenizer)
        ...
        print("Raw mrr: ", np.mean(1.0 / np.array(ranks)))
        for i in [0,2,9]:
