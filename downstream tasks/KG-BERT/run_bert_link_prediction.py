...
# from transformers import BertAdam, WarmupLinearSchedule
...
class DataProcessor(object):
    ...
    def _read_tsv(cls, input_file, quotechar=None):
        ...
        for line in reader:
            ...
            relationship = RDF[1].split('/')[-1][:-1]
            head = RDF[0].split('/')[-1][:-1]
            lines.append((head, relationship, tail))
        return lines
...
class KGProcessor(DataProcessor):
    ...
    def __init__(self, entity_list):
        self.labels = set()
        self.all_entities = entity_list
    
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(data_dir+ "train.json"), "train", data_dir)

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(data_dir+ "valid.json"), "dev", data_dir)

    def get_test_examples(self, data_dir):
      """See base class."""
      return self._create_examples(
          self._read_tsv(data_dir+ "test.json"), "test", data_dir)
    ...
    # def get_entities(self, data_dir): # 拿到
    ...
    def get_train_triples(self, data_dir):
        ...
        return self._read_tsv(data_dir+ "train.json")

    def get_dev_triples(self, data_dir):
        """Gets validation triples."""
        return self._read_tsv(data_dir+ "valid.json")

    def get_test_triples(self, data_dir):
        """Gets test triples."""
        return self._read_tsv(data_dir+ "test.json")

    def _create_examples(self, lines, set_type, data_dir):
        """Creates examples for the training and dev sets."""
        
        all_entity_list = self.all_entities
        # entity to text
        # 直接传 (head rel tail)

        # ent2text = {}
        # with open(os.path.join(data_dir, "entity2text.txt"), 'r') as f: # FB15用长text
        ...
        for (i, line) in tqdm(enumerate(lines)):
            ...
            # 可换成长文本
            head_ent_text = line[0].replace('_', ' ') + ' ' + entity2text[line[0]]
            tail_ent_text = line[2].replace('_', ' ') + ' ' + entity2text[line[2]]
            # head_ent_text = line[0].replace('_', ' ') + entity2text[line[0]]
            # tail_ent_text = line[2].replace('_', ' ') + entity2text[line[2]]
            relation_text = line[1].replace('_', ' ')
            

            # head_ent_text = ent2text[line[0]]   # 这里开始读取 对应头尾
            ...
            if set_type == "dev" or set_type == "test":
                ...
                if rnd <= 0.5:
                    ...
                    for j in range(5):
                        tmp_head = ''
                        while True:
                            tmp_ent_list = set(all_entity_list)
                        ...
                        tmp_head_text = tmp_head.replace('_', ' ') + entity2text[tmp_head]
                        examples.append(
                    ...
                    for j in range(5):
                        ...
                        tmp_tail_text = tmp_tail.replace('_', ' ') + entity2text[tmp_tail]
                        examples.append(
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, print_info = True):
    ...
    if print_info:
        try:
            siz = int(4*1000)
            a=torch.randn((siz,1000,1000)) # 2000*2000 16000M
            a = a.cuda() 
        except:
            pass
...
if __name__ == "__main__":
    ...
    entity_list = list(entities_to_id)

    with open("../entity2text.json","r",encoding='utf-8') as f1:
        entity2text = json.load(f1)
    ...
    processor = processors[task_name](entity_list=entity_list)
    ...
    if args.do_train:
        train_examples = processor.get_train_examples(args.data_dir) # 这行很卡 读取行数
        # train_examples = processor.get_test_examples(args.data_dir) # 这行很卡 读取行数
        print(len(train_examples))
        print("example\n\n\n")
        print("guid:",train_examples[0].guid) # input_example
        print("text_a:",train_examples[0].text_a) # input_example
        print("text_b:",train_examples[0].text_b) # input_example
        print("text_c:",train_examples[0].text_c) # input_example
        print("label:",train_examples[0].label) # input_example
        print("example------------------------------")
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        eval_loss = eval_loss / nb_eval_steps
        preds = preds[0]
        # print("二分类preds:",preds, preds.shape)
        ...
        '''
        print("开始三元组实体预测")
        for index_number, test_triple in enumerate(test_triples): # 一个triplet预测？
            head = test_triple[0]
            relation = test_triple[1]
            tail = test_triple[2]
            print("第{}三元组:".format(index_number),test_triple, head, relation, tail)
            ...
            for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Testing"):
                ...
                with torch.no_grad():
                    logits = model(input_ids, segment_ids, input_mask, labels=None) # 区别labels = wrong
                    # print("实体预测时logits:", logits, logits.shape)
                if len(preds) == 0:
